...
import { ContentContainer, HomeContainer, PostContainers } from './styles';
    ...
    <HomeContainer>
      ...
      {posts && posts.length > 0 && (
        <>
          ...
          <ContentContainer>
            <Filters categories={categories} authors={authors} lateral={true} />

            <PostContainers>
              {filteredPosts.length === 0 && <div>No posts found</div>}
              {filteredPosts.length > 0 && filteredPosts.map((post: Post) => <PostCard post={post} key={post.id} />)}
            </PostContainers>
          </ContentContainer>
        </>
      )}
